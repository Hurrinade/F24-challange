...
npm run lint:fix     # Run lint checks and auto-fix
npm run format       # Format project files
npm run typecheck    # Check for ts errors
npm run check        # Lint, typecheck and prettier check without mutating files
```
...
- Reference Convex docs for the database and API layer.
...
- The special root route `/` is the only exception and should be a direct file:
  `src/pages/Root.tsx`
- All other pages should be placed directly under `src/pages` in a folder that
  mirrors the route segment.
- Nested routes should mirror route segments with kebab-case folders.

Examples

- `Root.tsx` is the page for `/`
- `Folder.tsx` is in folder `folders` because route is `/folders/:folderId`
- `FolderDetail.tsx` would be in folder `folders/folder-detail` because route is `/folders/folder-detail`
...
- Keep `vite-plugin-pwa` metadata specific to File Browser and replace the
  placeholder PWA assets before production deployment.

## Legal, Privacy, GDPR

- For real SaaS, startup, or portfolio apps, add a basic
...
- Privacy and terms text must match the actual project, providers, cookies,
  analytics, database, payments, email, and hosting setup. Do not copy
...
- Always run `npm run check` when you finish adding code
